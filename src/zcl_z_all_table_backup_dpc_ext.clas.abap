class ZCL_Z_ALL_TABLE_BACKUP_DPC_EXT definition
  public
  inheriting from ZCL_Z_ALL_TABLE_BACKUP_DPC
  create public .
...
CLASS ZCL_Z_ALL_TABLE_BACKUP_DPC_EXT IMPLEMENTATION.
...
METHOD /iwbep/if_mgw_appl_srv_runtime~execute_action.
  ...
  IF iv_action_name = 'ExportTables'.
    ...
    " Get the 'TableNames' parameter value sent by the user in the URL.
    ...
    " Get the 'FilePath' parameter value sent by the user in the URL.
    ...
    " Check if the user's input is empty or just contains spaces.
    ...
    IF lv_clean_tables IS INITIAL.
      ...
      " Send the result back to the user and stop.
    ...
    " This special variable is needed because the function module requires an older data type.
    DATA lv_fm_path TYPE epsf-epsdirnam.
    lv_fm_path = lv_validated_path.

    " Call a standard SAP function to safely check if the folder exists on the server.
    ...
    REPLACE ALL OCCURRENCES OF '"'  IN lv_check_wildcard WITH ''.
    TRANSLATE lv_check_wildcard TO UPPER CASE.

    " Check if the user sent a wildcard pattern (contains '*')
    IF lv_check_wildcard CS '*'.

      " Handle special case: '*' means all Z and Y tables
      IF lv_check_wildcard = '*'.
        " Get all Z* and Y* tables from SAP dictionary
        SELECT tabname
          FROM dd02l
      ...
      ELSE.
        " Validate that pattern starts with Z or Y for other wildcards
        IF lv_check_wildcard(1) <> 'Z' AND lv_check_wildcard(1) <> 'Y'.
          ls_result-status  = 'FAILED'.
          ls_result-message = 'Wildcard patterns must start with Z or Y only. Other letters are not allowed.'.
          APPEND ls_result TO lt_results.
          copy_data_to_ref( EXPORTING is_data = lt_results CHANGING cr_data = er_data ).
          RETURN.
        ENDIF.
        ...
        DATA(lv_like_pattern) = lv_check_wildcard.
        REPLACE ALL OCCURRENCES OF '*' IN lv_like_pattern WITH '%'.

        " Get all tables matching the pattern from SAP dictionary
        SELECT tabname
          FROM dd02l
          WHERE tabname LIKE @lv_like_pattern
            AND as4local = 'A'
            AND tabclass = 'TRANSP'
          INTO TABLE @lt_tables.
      ENDIF.
    ...
    ELSE.
      " If no wildcard, split the comma-separated list of table names into a table.
    ...
    IF lt_tables IS INITIAL.
      ...
      ls_result-message = 'No tables found matching the specified pattern.'.
      APPEND ls_result TO lt_results.
      copy_data_to_ref( EXPORTING is_data = lt_results CHANGING cr_data = er_data ).
      RETURN.
    ENDIF.

    "Start the main loop to process each table in the list.
    " Prepare a clean result structure for the current table.
    LOOP AT lt_tables ASSIGNING <fs_table>.
      CLEAR ls_result.
      ls_result-tablename = <fs_table>.
      CLEAR ls_result-filepath.

      " Security check: only allow custom tables (even for wildcard results, double check)
      ...
      " Check if the table really exists in the SAP Data Dictionary.
      ...
      TRY.
          " DYNAMIC PROGRAMMING: Create a new internal table in memory that has the exact
          " same structure as the table the user specified. This is very flexible.
      ...
      TRY.
          " DYNAMIC SQL: Read all data from the database table (whose name is only
          " known at runtime) into our newly created dynamic table.
      ...
      TRY.
          " RTTI (Run-Time Type Information): Get a description of the table's structure,
          " which includes information like all of its column names.
      ...
      " Get the list of column names from the structure description.
      DATA(lt_components) = lo_struct_descr->get_components( ).
      DATA lt_excel_data TYPE TABLE OF string.
      " Build the header row for the file by joining all column names with a tab character.
      ...
      " Loop through each row of data we got from the database.
      LOOP AT <fs_dyn_table> ASSIGNING <fs_line>.
        CLEAR lv_line.
        " Loop through each column for the current row.
        LOOP AT lt_components INTO DATA(ls_component).
          " DYNAMICALLY get the value of a specific column for the current row.
      ...
      " Open a new file on the server at the specified path, preparing it for writing.
    ...
    " Send the final table of results back to the user who called the service.
    copy_data_to_ref(
